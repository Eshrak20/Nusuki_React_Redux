        ...
        <section className="relative py-8 lg:py-16 px-2 overflow-hidden">
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between mb-6 gap-6">
                <div className="space-y-2">
                    ...
                    <h2 className="text-3xl md:text-4xl lg:text-5xl font-black text-slate-900 dark:text-white leading-tight whitespace-normal md:whitespace-nowrap">
                ...
                <div>
                    <p className="text-slate-500 dark:text-slate-400 max-w-sm text-sm lg:border-l-2 border-slate-200 dark:border-slate-800 lg:pl-4">
                        Hand-picked journeys selected by our travel experts for those
                    ...
                    {collections.map((item) => (
                        <SwiperSlide key={item.id} className="py-3 lg:py-8">
                            ...
                            >
                                ...
                                {/* Subtle Overlay */}
                                <div className="absolute inset-0 bg-slate-900/20 group-hover/card:bg-slate-900/10 transition-colors duration-500" />
                                ...
                                <div className="absolute bottom-6 left-6 right-6 bg-white/20 dark:bg-slate-900/30 backdrop-blur-xl border border-white/30 dark:border-white/10 p-6 rounded-[2rem] shadow-2xl transition-all duration-500 group-hover/card:bg-white/40 dark:group-hover/card:bg-slate-800/50">
                                    <div className="flex justify-between items-center gap-4">
                                        ...
                                        <div className="h-10 w-10 shrink-0 rounded-full bg-primary text-muted flex items-center justify-center -rotate-45 group-hover/card:rotate-0 transition-transform duration-500 shadow-lg shadow-primary/30">
                                ...
                                {/* Hover Glow Effect */}
                                <div className="absolute -inset-1 bg-primary/20 opacity-0 group-hover/card:opacity-100 blur-2xl transition-opacity duration-500 -z-10" />
                            </a>
                        </SwiperSlide>
                    ))}
                </Swiper>

                <button className="prev-coll hidden absolute -left-2.5 top-1/2 -translate-y-1/2 z-30 h-12 w-12 rounded-xl bg-white dark:bg-slate-900 shadow-2xl md:flex items-center justify-center text-slate-800 dark:text-white hover:bg-primary hover:text-white transition-all duration-300 border border-slate-100 dark:border-slate-800">
                    <ChevronLeft size={24} />
                </button>

                <button className="next-coll hidden absolute -right-2.5 top-1/2 -translate-y-1/2 z-30 h-12 w-12 rounded-xl bg-white dark:bg-slate-900 shadow-2xl md:flex items-center justify-center text-slate-800 dark:text-white hover:bg-primary hover:text-white transition-all duration-300 border border-slate-100 dark:border-slate-800">
