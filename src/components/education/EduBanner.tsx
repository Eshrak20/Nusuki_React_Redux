import bannerVideo from "../../../src/assets/reactAssets/Education/32975-394513987.mp4";

const EduBanner = () => {
  return (
    <div className="relative h-75 mt-22 w-full overflow-hidden flex flex-col justify-center items-center px-8 md:px-20 text-center">
      <video autoPlay loop muted playsInline className="absolute top-0 left-0 w-full h-full object-cover z-0">
        <source src={bannerVideo} type="video/mp4" />
      </video>
      <div className="absolute top-0 left-0 w-full h-full bg-black/20 z-10"></div>

      {/* Added items-center to the container and text-center to the text block */}
      <div className="relative z-20 text-white mb-12 flex flex-col items-center">
        <h1 className="text-3xl lg:text-5xl font-extrabold mb-3">
          Welcome to Nusuki Education!
        </h1>
        <p className="text-lg lg:text-xl opacity-90 max-w-2xl">
          Choose your destination, find your dream institution and get abroad courses & tests at the best price
        </p>
      </div>
    </div>
  );
};

export default EduBanner;