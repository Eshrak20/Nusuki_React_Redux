...
const RefundabilityFilter = ({ data }: Props) => {
  ...
  const selected = useSelector((state: RootState) => state.flightSearch.filters.refundability);
  ...
  return (
    <Card className="rounded-2xl border-0 bg-white shadow-none">
      <CardHeader className="pb-3">
        <CardTitle className="text-base">Refundability</CardTitle>
      </CardHeader>
        ...
        {data.map((item) => (
          <label key={item.value} className="flex cursor-pointer items-center gap-3 rounded-xl border px-3 py-3">
            <Checkbox
              ...
              onCheckedChange={() => dispatch(updateFilter({ category: "refundability", value: item.value }))}
            />
