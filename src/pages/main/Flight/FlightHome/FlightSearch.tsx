...
const FlightSearch = ({ searchDests }: searchDestsProps) => {
    ...
    const defaultReturnDay = useMemo(() => addDays(nextDay, 2), [nextDay]);
    
    const [departureDate, setDepartureDate] = useState<Date | undefined>(nextDay);
    const [returnDate, setReturnDate] = useState<Date | undefined>(defaultReturnDay);
                ...
                <div className="flex flex-wrap gap-2 md:gap-4">
                    {["one-way", "round-way", "multi-way"].map((type) => (
                        <button
                            ...
                            className={`flex items-center gap-2 px-4 py-2 rounded-md border transition-colors text-sm font-medium ${tripType === type ? "bg-primary text-muted border-primary" : "bg-transparent text-slate-700 dark:text-primary border-slate-300"}`}
                        >
                            <div className={`w-4 h-4 rounded-full border flex items-center justify-center ${tripType === type ? "dark:bg-black border-white" : "border-slate-400"}`}>
                    ...
                    {activeDropdown === "traveler" && (
                        <div className="absolute top-[110%] right-0 w-72 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg shadow-2xl z-[110] p-4">
                            <div className="space-y-4">
                                {/* Adult, Child, Infant counters remain the same */}
                                        ...
                                        <div className="absolute left-0 right-0 top-full mt-1 bg-white border rounded-md shadow-lg z-[130] overflow-hidden">
                                            {flightClasses.map((cls) => (
                                                <div
                                                    ...
                                                    className={`px-4 py-2 text-sm cursor-pointer transition-colors ${flightClass === cls ? "bg-primary dark:bg-[#13275f] text-muted" : "hover:bg-slate-100 text-slate-700"}`}
                                                >
                                                    {cls}
                                                </div>
                                            ))}
                                ...
                                <button
                                    ...
                                    className="w-full bg-primary text-muted py-2 rounded-md font-bold text-sm hover:opacity-90 transition-colors"
                                >
                                    Done
                                </button>
                            </div>
                ...
                <div className="flex flex-col md:flex-row w-full lg:w-auto flex-1 gap-2 relative">
                    ...
                    <div
                        ...
                        onClick={() => {
                            ...
                            setIsReturnCalendarOpen(false); // Close the other calendar if open
                        }}
                    >
                    ...
                    {tripType !== "one-way" ? (
                        <div
                            className={`flex-1 border rounded-md p-3 cursor-pointer transition-all ${isReturnCalendarOpen ? 'border-primary ring-1 ring-primary' : 'border-slate-300 dark:border-slate-700 hover:border-primary'}`}
                            onClick={() => {
                        ...
                        >
                            ...
                            {/* Floating Return Calendar */}
                            {isReturnCalendarOpen && (
                                <div
                                    ref={returnCalendarRef}
                                    className="absolute top-[110%] left-0 md:left-auto md:right-0 z-[150] bg-white dark:bg-slate-900 shadow-2xl border border-slate-200 dark:border-slate-800 rounded-xl p-2"
                                    onClick={(e) => e.stopPropagation()}
                                >
                                    <DayPicker
                                        mode="single"
