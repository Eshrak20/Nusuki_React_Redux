...
import { setHotelPriceFilter } from "@/redux/features/hotel/hotelSearchSlice";
import FlightFilterSection from "../../Flight/FlightDetail/filters/reusableComponents/FlightFilterSection";
...
interface HotelPriceRangeFilterProps {
  data?: PriceRangeData;
  isLoading?: boolean;
  onChange?: () => void;
}
...
const HotelPriceRangeFilter = ({
  ...
  isLoading = false,
  onChange,
}: HotelPriceRangeFilterProps) => {
  const dispatch = useDispatch();

  const selectedMin = useSelector(
    (state: RootState) => state.hotelSearch.filters.price_min,
  );

  const selectedMax = useSelector(
    (state: RootState) => state.hotelSearch.filters.price_max,
  );
  ...
  if (isLoading) return <PriceRangeFilterSkeleton />;
